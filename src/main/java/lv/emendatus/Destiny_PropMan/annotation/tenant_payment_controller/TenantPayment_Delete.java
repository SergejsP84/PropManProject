...
@Operation(
        summary = "Delete a tenant's payment by ID",
        description = "NOT accessible in the current setup, but retained for possible future use." +
                "Removes a tenant's payment from the database",
        tags = {"OTHER"},
        parameters = {
                @Parameter(
                        name = "id",
                        description = "The unique identifier of the tenant's payment",
                        required = true,
                        in = ParameterIn.PATH,
        ...
        responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "The tenant's payment has been deleted"
                ),
                @ApiResponse(
                        responseCode = "400",
                ...
                @ApiResponse(
                        responseCode = "404",
                        description = "Tenant's payment not found",
                        content = @Content(
                                mediaType = "application/json",
                                schema = @Schema(implementation = ErrorResponse.class)
                        )
...
public @interface TenantPayment_Delete {
