...
@Operation(
        ...
        description = "Removes a manager specified by managerId after performing a thorough check across the database for any references to the tenant being removed and deleting or replacing the respective entity fields with stubs. This endpoint is accessible only by admins.",
        tags = {"ADMIN_FUNCTION"},
        parameters = {
                @Parameter(name = "managerId", description = "ID of the manager to be removed", required = true, in = ParameterIn.PATH)
        },
        responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "Manager with ID {managerId} has been removed.",
                        content = @Content(
                                mediaType = "application/json",
                                schema = @Schema(implementation = String.class),
                                examples = @ExampleObject(value = "Manager with ID 1 has been removed.")
                        )
                ),
                @ApiResponse(
                        responseCode = "401",
                ...
                @ApiResponse(
                        ...
                        content = @Content(
                                mediaType = "application/json",
                                schema = @Schema(implementation = String.class),
                                examples = @ExampleObject(value = "{\"message\": \"Failed to remove manager: error message\"}")
                        )
                )
        },
        security = {
...
public @interface AdminFunc_RemoveManager {
